...
from .schemas import ChatRequest, GenerationUpdate, ModelUpdate, ToolActiveUpdate


def build_router(*, manager: AgentManager, samples: list[dict], static_dir: Path) -> APIRouter:
    router = APIRouter()
    ...
    @router.post("/api/tools/{name}/active")
    ...
    async def chat_stream(request: ChatRequest = Body(...)) -> EventSourceResponse:
        ...
        async def event_generator() -> AsyncGenerator[dict, None]:
            async for event in session.stream_run(request.message):
                if event["type"] == "text":
                    for chunk in chunk_text(event["content"]):
                        yield {"data": chunk}
